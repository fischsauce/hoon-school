...
:: Intro to Hoon Type Inference
    ...
    :: Using Conditionals for Inference by Branch
        ...
        :: ?~ Null Match Test
            ...
            :: The ?~ rune is a lot like ?@ and ?^. 
            
            :: It takes three subexpressions, the first of which is evaluated to see whether the result is null, ~. If so, the second subexpression is evaluated. Otherwise, the third one is evaluated.
        ...
        :: Using ?~ With Lists
            ...
            :: The ?~ is especially useful for working with lists. Is a list null, or not? 
            
            :: You probably want to do different things based on the answer to that question. 
    ...
    :: Other Kinds of Type Inference
        ...
        :: So far you've learned about four kinds of type inference, using: 

            :: (1) literals, 
            :: (2) explicit casts, 
            :: (3) gate sample definitions, and 
            :: (4) branch specialization using runes in the ? family.

        :: In fact, there are several other ways that Hoon infers type:
        

        :: Any rune expression that evaluates to a flag, ?, e.g., .=, will be inferred from accordingly. 
        
            :: The .+ rune always evaluates to an @, and Hoon knows that too. The cell constructor runes, :-, :+, :^, and :* are all known to produce cells.



            :: More subtly, the =+, =/, and =| runes modify the subject by pinning values to the head. 
            
            :: Hoon infers from this that the subject has a new type: a cell whose head is the type of the pinned value and whose tail is the type of the (old) subject.


        :: (!) In general, anything that modifies the subject modifies the type of the subject (!)
        
        :: Type inference can work in subtle ways for various expressions. However, we have covered enough that it should be relatively clear how to anticipate how type inference works for the vast majority of ordinary use cases.
