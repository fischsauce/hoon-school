...
:: 2.3.1 Walkthrough: Libraries
    ...
    :: In Hoon, like in other languages, it's often useful to create a library for other code to access. 

    :: In this example, we look at one example of a library that can be used to represent a deck of 52 playing cards. 

    :: The core below builds such a library, and can be accessed by programs.
...
+$  deck  (list darc)
    ...
    :: One way to get a feel for how a library works is to skim the ++ arm-names before diving into any specific arm. 
    
    :: In this library, the arms are make-deck, num-to-suit, shuffle-deck, and draw. These names should be very clear, with the exception of num-to-suit though you could probably hazard a guess at what it does. 


:: num-to-suit is used in the make-deck arm. This arm should be quite readable to you at this point. 

:: Here we simply have two loops where we use counters to build up the full set of 52 cards, by cycling through every possible suit and number and combining them. 

:: Once we have reached the point where j is greater than 14, we're going to jump back out to the "suit" outer-loop and increment i. 

:: ?. may be an unfamiliar rune; it is simply the inverted version of ?:, so the first branch is actually the "no" branch and the second is the "yes" branch. This is done to keep the "heaviest" branch at the bottom.
++  make-deck
  ^-  deck
  =|  mydeck=deck
  =/  i  1
...
:: The ?+ rune is the rune to switch against a value with a default. The default here is to crash with !!. 

:: Then we have the four options of 1 through 4, based on the value of val, which each resulting in a different suit:
++  num-to-suit
  |=  val=@ud
  ^-  suit
  ?+  val  !!
...
:: This gate takes two arguments: a deck, and a @ as a bit of entropy to seed the og core. It's going to produce a deck.
...
:: With that completed, we use %= to call $ to recurse back up to |- with a few changes:
    ...
    :: 'unshuffled' becomes the result of using oust to remove 1 'darc' at index on 'u'nshuffled'.
...
:: 'draw' takes two arguments: n, a number; and d, a deck. 

:: It's going to produce a cell of two decks. The cell is created using 'scag' and 'slag'. 

:: scag is a standard-library gate produces the first n elements from a list, and slag is a standard-library gate that produces the remaining elements of a list starting after the nth element. 

:: So we use scag to produce the drawn hand of n cards in the head of the cell as 'hand', and slag to produce the remaining 'deck' in the tail of the cell as rest.
++  draw
  |=  [n=@ud d=deck]
  ^-  [hand=deck rest=deck]
  :-  (scag n d)
